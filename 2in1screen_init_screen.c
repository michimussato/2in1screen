/*
mkdir -p ./bin \
&& gcc -O2 -o ./bin/2in1screen_init_screen 2in1screen_init_screen.c \
&& chown root:root ./bin/2in1screen_init_screen \
&& chmod 0755 ./bin/2in1screen_init_screen \
&& sudo mv ./bin/2in1screen_init_screen /usr/bin/
*/
...
void rotate_screen(){
    ...
    // ROTATE DISPLAY
	sprintf(command, "xrandr --output %s --rotate %s", output, ROT[state]);
	system(command);
	...
	// libwacom
	sprintf(command, "xsetwacom set \"%s\" \"Rotate\" %s", "GXTP7380:00 27C6:0113 touch", ROT_WACOM[state]);
	system(command);
	sprintf(command, "xsetwacom set \"%s\" \"Rotate\" %s", "GXTP7380:00 27C6:0113 Stylus stylus", ROT_WACOM[state]);
	system(command);
	sprintf(command, "xsetwacom set \"%s\" \"Rotate\" %s", "GXTP7380:00 27C6:0113 Stylus eraser", ROT_WACOM[state]);
	system(command);
    ...
    // libinput (only?)
	sprintf(command, "xinput set-prop \"%s\" --type=float \"Coordinate Transformation Matrix\" %s", "HAILUCK CO.,LTD USB KEYBOARD Mouse", COOR_TOUCHPAD[state]);
	system(command);
}
